...
جدول `verses` حاوی اطلاعات شعری و متنی است. فیلدهای آن به شرح زیر هستند:
...
### فیلدهای Tag (نسخه HTML-tagged)

- `verse_1_tag` : مصراع اول (`verse_1`) که با تگ‌های HTML خاص نگهداری می‌شود. این فیلد از نوع `Text` است و در جدول پایگاه داده به‌صورت دستی افزوده شده است (NOT NULL).
- `verse_2_tag` : مصراع دوم (`verse_2`) که با تگ‌های HTML خاص نگهداری می‌شود. این فیلد از نوع `Text` است و اختیاری است.

توجه: این دو فیلد به‌صورت دستی به جدول `verses` در دیتابیس اضافه شده‌اند و در حال حاضر نیازی به تعریف مایگرِیشن جدید نیست. کافیست مدل در `app/models.py` شامل این فیلدها باشد تا SQLAlchemy آن‌ها را در سطح کد بشناسد.
